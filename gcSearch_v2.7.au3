...
#cs
	...
	作者:        Jack Chen
	网站:        https://github.com/cnjackchen/gcSearch
...
Global $IndexTimer, $hIndexTimer, $hTimer
...
;Global $CMDCtrlEdit = "WindowsForms10.EDIT.app.0.378734a1" ; 气相 WindowsForms10.EDIT.app.0.378734a；液相 WindowsForms10.EDIT.app.0.33c0d9d
Global $CMDCtrlEdit = "[REGEXPCLASS:(?i)WindowsForms10.EDIT.app.0; INSTANCE:1]"
...
$IndexTimer = IniRead($SettingsFile, "Settings", "IndexTimer", 0) * 1 ; 更新索引时间间隔 hours
...
EndIf
If $IndexTimer > 0 Then
	$hTimer = TimerInit()
	AdlibRegister("IndexTimer", 10000)
EndIf
...
Func _FileInfoSave()
	...
	_SQLite_Exec($hDb, "REPLACE INTO Index1 VALUES ('" & $aIndex[0] & "','" & $aIndex[1] & "','" & _
			$aIndex[2] & "','" & $aIndex[3] & "','" & $aIndex[4] & "','" & $aIndex[5] & "','" & $aIndex[6] & "','" & _
...
Func IndexTimer()
	If $IsRebuilding = True Then Return

	If TimerDiff($hTimer) > ($IndexTimer * 60 * 60 * 1000) Then
		$hTimer = TimerInit()
		DoIndex(0)
	EndIf
EndFunc

Func DoIndex($Rebuild)
	Local $t, $hIndexDb, $i, $time
	$t = TimerInit()
	...
	Else
		...
		_GUICtrlStatusBar_SetText($hStatus, "正在更新索引，请稍候...", 2)
	EndIf
	$hIndexDb = _SQLite_Open($IndexFile)

	_SQLite_Exec($hIndexDb, "CREATE TABLE IF NOT EXISTS Index1 (样品名称 TEXT,分析方法 TEXT,分析日期 TEXT," & _
			"分析时间 TEXT,数据文件 TEXT UNIQUE,操作者 TEXT,样品信息 TEXT,更新时间 TEXT);")
	...
	For $i = 1 To $aDirs[0]
		If FileExists($aDirs[$i]) Then
			_IndexDir($aDirs[$i], $hIndexDb, $IndexCount, $Rebuild, $time)
		EndIf
	Next
	_SQLite_Exec($hIndexDb, "DELETE FROM Index1 WHERE 更新时间<'" & $time & "';")
	_SQLite_Exec($hIndexDb, "COMMIT;")
	_SQLite_Close($hIndexDb)

	$t = Round(TimerDiff($t) / 60000, 2)
	_ShowIndexStatus($IndexFile, $hStatus)
	_GUICtrlStatusBar_SetText($hStatus, "索引已更新，用时 " & $t & " 分钟。", 2)
	ReduceMemory()
EndFunc   ;==>DoIndex


Func _IndexDir($sDir, $hIndexDb, ByRef $IndexCount, $Rebuild, $time = 0)
	...
	Do
		...
		If $search <> -1 Then
			While 1
				$fname = FileFindNextFile($search)
				If @error = 1 Then ExitLoop
				$fname = $aDir[$iCnt] & "\" & $fname
				If @extended <> 1 Then
					If $Rebuild = 0 Then
						If FileIndexed($fname) = 1 Then
							_SQLite_Exec($hIndexDb, "UPDATE Index1 SET 更新时间='" & $time & "' WHERE 数据文件='" & $fname & "';")
							ContinueLoop
						EndIf
					EndIf
					$aIndex = _IndexFile($fname)
					If $time = 0 Then
						_SQLite_Exec($hIndexDb, "REPLACE INTO Index1 VALUES ('" & $aIndex[0] & "','" & $aIndex[1] & "','" & _
								$aIndex[2] & "','" & $aIndex[3] & "','" & $aIndex[4] & "','" & $aIndex[5] & "','" & $aIndex[6] & "','" & _
								@YEAR & @MON & @MDAY & @HOUR & @MIN & @SEC & "');")
					Else
						_SQLite_Exec($hIndexDb, "REPLACE INTO Index1 VALUES ('" & $aIndex[0] & "','" & $aIndex[1] & "','" & _
								$aIndex[2] & "','" & $aIndex[3] & "','" & $aIndex[4] & "','" & $aIndex[5] & "','" & $aIndex[6] & "','" & $time & "');")
					EndIf
					$IndexCount += 1
...
Func FileIndexed($file)
	Local $aResult, $iRows, $iColumns, $iRval

	$iRval = _SQLite_GetTable(-1, "SELECT 样品名称,分析日期,分析时间 FROM Index1 WHERE 数据文件='" & $file & "';", $aResult, $iRows, $iColumns)
	...
	If $aResult[1] <> "" Then Return 1 ;样品名称不空，已索引

	;样品名称空但分析时间在一天前，已索引
	If _DateDiff('d', $aResult[2] & " " & $aResult[3], _NowCalc()) > 1 Then
		Return 1
	EndIf
EndFunc
...
Func _GetChemID()
	Local $ChID = WinGetHandle("[REGEXPCLASS:(?i)WindowsForms10; REGEXPTITLE:(?i)(?:Offline|脱机)]")
	Return $ChID
EndFunc   ;==>_GetChemID
...
Func GUI_Settings($TabNo = 0)
	...
	GUICtrlCreateLabel("每隔",15, 295, 30, 20)
	$hIndexTimer = GUICtrlCreateInput($IndexTimer, 45, 293, 40, 20)
	GUICtrlCreateLabel("小时自动更新索引（设为0关闭自动更新）。", 90, 295, 400, 20)
...
Func GUI_SettingsOK()
	...
	$IndexTimer = GUICtrlRead($hIndexTimer)*1
	IniWrite($SettingsFile, "Settings", "IndexTimer ", $IndexTimer)

	If $IndexTimer > 0 Then
		$hTimer = TimerInit()
		AdlibRegister("IndexTimer", 10000)
	Else
		AdlibUnRegister("IndexTimer")
	EndIf
...
Func IndexNewFile()
	...
	For $i = 0 To UBound($arr) - 1
		...
		_SQLite_Exec($hDb, "REPLACE INTO Index1 VALUES ('" & $aIndex[0] & "','" & $aIndex[1] & "','" & _
				$aIndex[2] & "','" & $aIndex[3] & "','" & $aIndex[4] & "','" & $aIndex[5] & "','" & $aIndex[6] & "','" & @YEAR & @MON & @MDAY & @HOUR & @MIN & @SEC & "');")
